...
此应用模板提供了快速创建SLB和多台ECS服务器组成Web后端环境的方案。部署此应用时， 
用户只需要输入ECS实例密码等少量参数，即可一键部署支持负载均衡和弹性伸缩的Web后端环境。

部署完成的应用分组包含一个或多个ECS实例、一个SLB实例和一个弹性伸缩组，ECS集群支持弹性扩缩容并自动挂载到负载均衡。 
待应用部署完成后，您会获得一套支持负载均衡和弹性伸缩的ECS集群环境，您可在创建的ECS实例中部署您的软件。
